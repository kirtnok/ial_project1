...
int Queue_IsFull( const Queue *queue ) {
	return (queue->firstIndex == nextIndex(queue->freeIndex)); //ak by sa mal prvy index rovnat dalsiemu free indexu fronta je plna
}
...
void Queue_Enqueue( Queue *queue, char data ) {
	...
	queue->freeIndex = nextIndex(queue->freeIndex); //posun indexu volneho miesta
}
